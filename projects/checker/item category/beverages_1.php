...
<?php
	...
	$product_array = $db_handle->runQuery("SELECT menuitem_id, item_category, name, item_regularprice, item_availability, code FROM menu_db where item_category ='".$_GET["cat"]."'");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item itmbtn btn" data-itemnumber="<?php echo $product_array[$key]["code"]; ?>" style="float:left">

			<form method="post" id="frm<?php echo $product_array[$key]["code"]; ?>" action="../dash_1.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>" target="iframe_menu">
			...
			<div><strong>
			<?php echo $product_array[$key]["item_regularprice"]; ?>
			</strong></div>
